...
(define size-dim-1 
  (lambda (mat)
	(let ((size (- (vector-length mat) 2)))
	  (/ size (vector-ref mat (1+ size))))))

(define size-dim-2
  (lambda (mat)
	(vector-ref mat (- (vector-length mat) 2))))

(define size-dim-3
  (lambda (mat)
	  (/ (vector-ref mat (1- (vector-length mat)))
		 (vector-ref mat (- (vector-length mat) 2)))))

(define array-ref
  (lambda (array)
	(let ((block-size (* (size-dim-2 array) (size-dim-3 array))))
	  (lambda (i j k)
		(vector-ref array (+ (* i block-size)
					   (* j (size-dim-3 array))
					   k))))))
...
(define array-set!
  (lambda (array)
	(lambda (i j k obj)
		(vector-set! array 
					 (+ (* i block-size)(* j (size-dim-3 array)) k)
					 obj))))

(let ((A (vector 6 5 4 3 1 2 9 7 8 2 1 0 2 6)))
  ((array-ref A) 1 1 1))
...
(define array-generator 
  (lambda (gen-proc)
	(lambda (dim1 dim2 dim3)
	  (let ((size (* dim1 dim2 dim3)) 
			(block-size (* dim2 dim3)))
		(let ((vec-gen-proc
				(lambda (ind)
				  (if (< ind size)
					(let ((i (quotient ind block-size))
						  (aux (remainder ind block-size)))
					  (gen-proc i
								(quotient aux dim3)
								(remainder aux dim3)))
					(if (= ind size)
...
((array-generator (lambda (i j k) i)) 2 2 3)
